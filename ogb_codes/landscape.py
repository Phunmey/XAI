...
def landscape_train(graph_data, graph_ids, graph_label, dataset="molhiv", data_type="train"):  # this is for the train test
    train_landscape = []
    ...
    for graph_id, label in zip(graph_ids, graph_label):
        ...
        train_landscape.append(
            [dataset] + [graph_id] + [graph_label] + [num_nodes] + [num_edges] + [landscape_time] + land_scape.tolist())
    ...
    columnnames.update(
        {(list(columnnames))[0]: 'dataset', (list(columnnames))[1]: 'graphId', (list(columnnames))[2]: 'graphLabel',
         ...
         (list(columnnames))[5]: 'landscapeTime', (list(columnnames))[6]: 'landscapeList'})
...
if __name__ == '__main__':
    ...
    train_features = landscape_train(graph_data, train_ids, train_labels, dataset="molhiv", data_type="train")
    val_features = landscape_train(graph_data, val_ids, val_labels, dataset="molhiv", data_type="val")
    test_features = landscape_train(graph_data, test_ids, test_labels, dataset="molhiv", data_type="test")

    train_features.to_csv(
        "/home/taiwo/projects/def-cakcora/taiwo/src/ogbMolhiv/ogbmolhiv_landscape_features/train_landscape_features.csv", index=False)
    val_features.to_csv(
        "/home/taiwo/projects/def-cakcora/taiwo/src/ogbMolhiv/ogbmolhiv_landscape_features/val_landscape_features.csv", index=False)
    test_features.to_csv(
        "/home/taiwo/projects/def-cakcora/taiwo/src/ogbMolhiv/ogbmolhiv_landscape_features/test_landscape_features.csv", index=False)
